...
public class StaffData implements NhanVienModelInterface {
	private String[] columnNames = { "", "Mã nhân viên", "Tên nhân viên", "Điện thoại", "Lương", "Chức vụ", "Email",
			"Giới tính", "Hoạt động", "Ghi chú", "Chi tiết" };
	...
	public void setData() {
		...
		for (int i = 0; i < rowData.length; i++) {
			...
			Object[] element = { false, nv.getId(), nv.getTen(), "0" + nv.getSdt(), nv.getLuong(), nv.getChucVu(),
					nv.getEmail(), sexString, statusString, nv.getGhiChu(), "Xem" };
			rowData[i] = element;
		}
	}
	...
	public boolean addNhanVien(String id, String ten, int sdt, int trangThai, String ghiChu, int gioiTinh, String email,
		...
		// TODO Auto-generated method stub
		boolean r = NhanVienDao.getInstance().addNhanVien(id, ten, sdt, trangThai, ghiChu, gioiTinh, email, diaChi,
				idChucVu);
		if (r)
			notifyObservers();
		return r;
	...
	public boolean setNhanVien(String id, String ten, int sdt, int trangThai, String ghiChu, int gioiTinh, String email,
			String diaChi, String idChucVu) {
		// TODO Auto-generated method stub
		boolean r = NhanVienDao.getInstance().setNhanVien(id, ten, sdt, trangThai, ghiChu, gioiTinh, email, diaChi,
				idChucVu);
		if (r)
			notifyObservers();
		return r;
