...
public class CollectDebtData implements ThuNoModelInterface{
	private String[] columnNames = { "", "Tên khách hàng","Mã Khách Hàng", "Số tiền", "Ngày thu", "Email", "Địa chỉ", "Ghi chú", "Trạng Thái" };
	...
	public void setData() {
		...
		for(int i=0;i<rowData.length;i++) {
			...
			String dateString;
			if(thuNo.getNgayThu()==null)dateString="";
			else {
			SimpleDateFormat sdf=new SimpleDateFormat("dd-MM-yyyy");
			 dateString=sdf.format(thuNo.getNgayThu());
			}
			Object[] element= {false,kh.getHoVaTen(),kh.getMaSo(),thuNo.getSoTienNo(),dateString,kh.getEmail(),kh.getDiachi(),kh.getGhiChu(),trangThai};
			rowData[i]=element;
		}
	}
	@Override
