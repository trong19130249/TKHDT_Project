...
public class MnOrders extends JPanel implements Observer{
	...
	private float[] columnWidthPercentage = { 0.03f, 0.06f, 0.1f,0.06f, 0.1f, 0.12f, 0.18f, 0.1f, 0.1f, 0.1f, 0.1f };
	double rateWidthTable = 1.0;

	public MnOrders(DonHangModelInterface data,HoaDonController con) {
		this.data=data;
		this.controller=con;
...
//		jTable.setDefaultRenderer(JButton.class, new JTableButtonRenderer(tableRenderer));
		new ButtonColumn1(jTable, 1,this);
...
//				new DetailAnOrder(new AnOrderData().getColumnNames(), new AnOrderData().getRowData());
		...
		btnFix.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0)new FixAnOrder(mn);
			}
		});
...
//			System.out.println(widthTable);
	...
	public boolean setDonHang(String idKh,Date ngayLap,String id,String idKhOld) {
		return controller.setDonHang(idKh, ngayLap,id,idKhOld);
	}
	public DonHang getDonHang(String id) {
		return data.getDonHang(id);
	}
	public boolean addDonHang(String idDh,String idKh,Date ngayLap) {
		return controller.addDonHang(idDh, idKh, ngayLap);
	}
	public Date getNgayLap() {
		return (Date) jTable.getModel().getValueAt(jTable.getSelectedRow(),6);
	}
