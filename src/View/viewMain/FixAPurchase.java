...
public class FixAPurchase extends JFrame {
	...
	public FixAPurchase(final MnPurchases mn) {
...
//		btCancel.setBorder(lineBorder);
		...
		btAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				LichSuMuaHang select=mn.getLichSu(mn.getId());
				Date ngayLap=select.getNgayNhap();
				String idNew=txtCode.getText().equals("")?mn.getId():txtCode.getText();
				String idnbh=ds.get(jcbName.getSelectedIndex()).getId();
				mn.setLichSu(idnbh,ngayLap, mn.getId(),mn.getIdNCC());
                dispose();
			}
