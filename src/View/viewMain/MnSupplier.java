...
public class MnSupplier extends JPanel implements Observer{
...
private NhaCungCapModelInterface data;
private NhaCungCapController controller;
	private float[] columnWidthPercentage = { 0.01f, 0.1f, 0.2f, 0.14f, 0.18f, 0.12f, 0.14f };

	public MnSupplier(NhaCungCapModelInterface data,final NhaCungCapController controller) {
		this.data=data;
...
//		panelBtns1.setBackground(Color.WHITE);
		...
		JLabel nameScreen = new JLabel("  Danh mục nhà cung cấp");
...
//		panelTableIn2.setBorder(lineBorder);
		...
		btnFix.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(jTable.getSelectedRow()>=0)new FixSupplier(mn);
			}
		});
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0) {
					controller.deleteNCC((String)jTable.getValueAt(jTable.getSelectedRow(), 1));
				}
