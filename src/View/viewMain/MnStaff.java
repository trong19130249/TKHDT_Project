...
public class MnStaff extends JPanel implements Observer {
...
//		panelBtns1.add(btnFix);
		...
		JLabel nameScreen = new JLabel("  Danh mục nhân viên");
		...
		for (int x = 0; x < jTable.getColumnCount(); x++) {
			if (jTable.getColumnModel().getColumn(x).getClass() != null && x != 8 && x != 0 && x != 6) {
				jTable.getColumnModel().getColumn(x).setCellRenderer(centerRenderer);
			}
		}
		new ButtonColumn4(jTable, 10,this);
...
//		panelTableIn2.setBorder(lineBorder);
		...
		btnFix.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0)	new FixStaff(mn);
			}
		});
	...
	public boolean addNhanVien(String id,String ten,int sdt,int trangThai,String ghiChu,int gioiTinh,String email,String diaChi,String idChucVu) {
		return data.addNhanVien(id, ten, sdt, trangThai, ghiChu, gioiTinh, email, diaChi, idChucVu);
	}
	public boolean setNhanVien(String id,String ten,int sdt,int trangThai,String ghiChu,int gioiTinh,String email,String diaChi,String idChucVu) {
		return data.setNhanVien(id, ten, sdt, trangThai, ghiChu, gioiTinh, email, diaChi, idChucVu);
	}
