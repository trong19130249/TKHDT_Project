...
public class MnCollectDebt extends JPanel implements Observer{
	...
	private ThuNoModelInterface data;
	private ThuNoController controller;
	private float[] columnWidthPercentage = { 0.03f, 0.2f,0.2f,0.3f, 0.3f, 0.4f, 0.52f, 0.4f, 0.4f};
	double rateWidthTable = 1.0;

	public MnCollectDebt(ThuNoModelInterface data,ThuNoController controller) {
		this.data=data;
...
//		panelBtns1.setBackground(Color.WHITE);
		...
		JLabel nameScreen = new JLabel("  Thao tác thu nợ");
...
//				btnAdd.setEnabled(false);
		...
		btnFix.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0)	new FixCollect(mn);
			}
		});
	...
	public boolean addDebt(String id, Date ngayThu,double soTienNo) {
		return data.addDebt(id, ngayThu, soTienNo);
	}
	public boolean deleteDebt(String idKh,Date ngayThu) {
		return data.deleteDebt(idKh, ngayThu);
	}
	public List<ThuNo> getAllDebt() {
		return data.getAllDebt();
	}
	public String getId() {
		return (String)jTable.getValueAt(jTable.getSelectedRow(), 2);
	}
	public boolean setDebt(String idKh, Date ngayThu, double soTienNo, String idOld) {
		boolean r=data.setDebt(idKh, ngayThu, soTienNo, idOld);
		return r;
	}
	public double getSoTien() {
		
		return (double)jTable.getValueAt(jTable.getSelectedRow(),3);
		
	}
	public Date getNgayThu() {
		//System.out.println((String)jTable.getValueAt(jTable.getSelectedRow(), 4));
		SimpleDateFormat sdf=new SimpleDateFormat("dd-MM-yyyy");
		java.util.Date date;
		try {
			date = sdf.parse((String)jTable.getValueAt(jTable.getSelectedRow(), 4));
			Date ngayThu=new Date(date.getTime());
			return ngayThu;
