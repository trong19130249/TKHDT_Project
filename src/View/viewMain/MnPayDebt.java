...
public class MnPayDebt extends JPanel implements Observer{
	...
	private TraNoModelInterface data;
	private TraNoController controller;
	private float[] columnWidthPercentage = { 0.02f, 0.3f,0.2f, 0.1f, 0.16f, 0.32f };
	double rateWidthTable = 1.0;

	public MnPayDebt(TraNoModelInterface data,TraNoController controller) {
		this.data=data;
...
//		panelBtns1.setBackground(Color.WHITE);
		...
		JLabel nameScreen = new JLabel("  Thao tác trả nợ");
...
//		panelTableIn2.setBorder(lineBorder);
		...
		btnFix.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0)new FixPay(mn);
			}
		});
		...
		btnDelete.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (jTable.getSelectedRow() >= 0)
					deleteDebt(getIdNBH(), getNgayTra());
			}
		});
	...
	public Date getNgayTra() {
		...
		try {
			...
			Date ngayTra=new Date(date.getTime());
			return ngayTra;
	...
	public boolean addDebt(String id, Date ngayTra,double soTienNo) {
		return controller.addDebt(id, ngayTra, soTienNo);
	}
	public boolean deleteDebt(String idNBH,Date ngayTra) {
		return controller.deleteDebt(idNBH,ngayTra);
	}
	public boolean setDebt(String idNbh,Date ngayTra, double soTienNo, String idOld) {
		return controller.setDebt(idNbh,ngayTra, soTienNo, idOld);
	}
}
