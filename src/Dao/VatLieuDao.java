...
public class VatLieuDao{
	...
	public ASanPham getVatLieu(String id) {
		...
		try {
			...
			PreparedStatement ps=connection.prepareStatement("select * from vat_lieu m inner join hinh_anh h on m.id=h.idvl where m.id=? and m.hd=1");
			ps.setString(1, id);
			...
			while(rs.next()) {
				if(!exits.contains(id)) {
					...
					result=MaterialFactory.getInstance().createMaterial(type, id, tenSanPham, mota, donViTinh, donGia, soLuong, list,giaNhap);
					exits.add(id);
				}
	...
	public List<ASanPham> getAll() {
		...
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("select * from vat_lieu m inner join hinh_anh h on m.id=h.idvl where m.hd=1");
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				...
				if(!exits.containsKey(maVL)) {
					...
					double giaNhap=rs.getDouble("gia_nhap");
					ASanPham vl=MaterialFactory.getInstance().createMaterial(type, maVL, tenSanPham, mota, donViTinh, donGia, soLuong, listUrl,giaNhap);
					exits.put(maVL, vl);
...
// Chỉnh sửa thông tin của vật liệu chỉ định dựa vào id truyền vào
	public boolean setProperty(String id,String ten,String donViTinh,double donGia,double soLuong,String loai,double giaNhap,String idnew) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("update vat_lieu m set m.don_gia=?,m.id=?,m.ten_vl=?,m.don_vi=?,m.don_gia=?,"
	...
	public boolean addVatLieu(String id,String ten,String donViTinh,double donGia,double soLuong,String loai,double giaNhap) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("insert into vat_lieu(id,ten_vl,mo_ta,don_vi,don_gia,so_luong_kho,loai,gia_nhap,hd)"
					+ " values(?,?,?,?,?,?,?,?,?)");
			ps.setString(1, id);
			ps.setString(2, ten);
			ps.setString(3, "");
	...
	// Xóa - Vô hiệu hóa vật liệu, việc này sẽ không xóa hoàn toàn vật liệu ra khỏi database mà chỉ vô hiệu hóa nó để có thể dễ dàng
