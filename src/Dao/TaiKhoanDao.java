...
public class TaiKhoanDao {
	...
	public TaiKhoan getAccountWithUser(String userName) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("select * from nhan_vien nv inner join tai_khoan tk on tk.id_nv=nv.id inner join chuc_vu cv on cv.id=nv.id_cv where tk.user_name=?");
	...
	public TaiKhoan getAccount(String id) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("select * from nhan_vien nv inner join tai_khoan tk on tk.id_nv=nv.id inner join chuc_vu cv on cv.id=nv.id_cv where tk.id_nv=?");
			ps.setString(1, id);
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				String ten=rs.getString("ten");
	...
	public List<TaiKhoan> getAll() {
		...
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("select * from nhan_vien nv inner join tai_khoan tk on tk.id_nv=nv.id inner join chuc_vu cv on cv.id=nv.id_cv");
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
	...
	public boolean addAccount(String idNhanVien,String userName,String pass,int admin) {
		try {
			...
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("insert into tai_khoan(id_nv,user_name,pass,admin) values(?,?,?,?)");
	...
	public boolean editAccount(String idNew,String id, String userName,String pass,int admin) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("update tai_khoan tk set tk.id_nv=?,tk.user_name=?,tk.pass=?,tk.admin=? where tk.id_nv=?");
