...
public class ThuNoDao {
	...
	public List<ThuNo> getAllDebt() {
		...
		try {
			...
			PreparedStatement ps = connection.prepareStatement(
					"select * from khach_hang kh inner join ls_thu_no ls on ls.id_kh=kh.id_kh",
	...
	public ThuNo getDebt(String idKh) {
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"select * from khach_hang kh inner join ls_thu_no ls on ls.id_kh=kh.id_kh where ls.id_kh=?",
					ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
			ps.setString(1, idKh);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public boolean addDebt(String id, Date ngayThu, double soTienNo) {
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("insert into ls_thu_no(id_kh,ngay_thu,so_tien_no) values(?,?,?)");
			ps.setString(1, id);
...
// tra no cho nha cc
	public boolean deleteDebt(String idKh, Date ngayThu) {
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection
					.prepareStatement("DELETE from ls_thu_no where ls_thu_no.ID_KH=? and ls_thu_no.ngay_thu=?");
			ps.setString(1, idKh);
			ps.setDate(2, ngayThu);
			int affect = ps.executeUpdate();
			if (affect > 0)
	...
	public boolean setDebt(String idKh, Date ngayThu, double soTienNo, String idOld) {
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"update ls_thu_no ls set ls.id_kh=? ,ls.so_tien_no=?,ls.ngay_thu=? where ls.id_kh=?");
			ps.setString(1, idKh);
