...
public class KhachHangDao {
	...
	public boolean setProperty(String id, String ten, String email, String diaChi, double soTienNo, String ghiChu) {
		try {
			...
			PreparedStatement ps = connection.prepareStatement(
					"update khach_hang k set k.ten=?,k.email=?,k.dia_chi=?,k.so_tien_no=?,k.ghi_chu=? where k.id_kh=?");
			ps.setString(1, ten);
	...
	public boolean addKhachHang(String id, String ten, String email, String diaChi, double soTienNo, String ghiChu) {
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"insert khach_hang(id_kh,email,ten,dia_chi,so_tien_no,ghi_chu,hd) values(?,?,?,?,?,?,?)");
			ps.setString(1, id);
			ps.setString(3, ten);
			ps.setString(2, email);
	...
	public List<KhachHang> getDanhSachKhachHang() {
		try {
			...
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				result.add(new KhachHang(rs.getString("id_kh"), rs.getString("ten"), rs.getString("dia_chi"),
						rs.getString("email"), rs.getDouble("so_tien_no"), rs.getString("ghi_chu")));
			}
			return result;
		} catch (SQLException e) {
	...
	public boolean deleteKhachHang(String id) {
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("update khach_hang set khach_hang.hd=0 where khach_hang.id_kh=?");
			ps.setString(1, id);
			int affect = ps.executeUpdate();
			if (affect > 0)
				return true;
