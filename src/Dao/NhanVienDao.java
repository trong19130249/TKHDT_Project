...
public class NhanVienDao {
	...
	public NhanVien getNhanVien(String id) {
		try {
			...
			PreparedStatement ps = connection.prepareStatement(
					"select * from nhan_vien nv inner join chuc_vu cv on cv.id=nv.id_cv where nv.id=?");
			ps.setString(1, id);
	...
	public List<NhanVien> getAll() {
		...
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("select * from nhan_vien nv inner join chuc_vu cv on cv.id=nv.id_cv");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
	...
	public boolean addNhanVien(String id, String ten, int sdt, int trangThai, String ghiChu, int gioiTinh, String email,
		...
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"insert into nhan_vien(id,ten,sdt,trang_thai,ghi_chu,gioi_tinh,email,dia_chi,id_cv) values(?,?,?,?,?,?,?,?,?)");
			ps.setString(1, id);
			ps.setString(2, ten);
			ps.setInt(3, sdt);
	...
	public boolean setNhanVien(String id, String ten, int sdt, int trangThai, String ghiChu, int gioiTinh, String email,
		...
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"update nhan_vien e set e.ten=?,e.sdt=?,e.trang_thai=?,e.ghi_chu=?,e.gioi_tinh=?,e.email=?,e.dia_chi=?,e.id_cv=? where e.id=?");
			ps.setString(1, ten);
			ps.setInt(2, sdt);
