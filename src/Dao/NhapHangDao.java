...
public static NhapHangDao getInstance() {
	if(instance==null) instance=new NhapHangDao();
	return instance;
}
public List<LichSuMuaHang> getAll(){
	...
	try {
		...
		PreparedStatement ps=connection.prepareStatement("select * from lich_su_nhap_hang ls inner join ct_lsnh ct on ct.id_ls=ls.id "
		...
		while(rs.next()) {
			...
			NhaBanHang nbh=new NhaBanHang(rs.getString("nbh.id"), rs.getString("nbh.ten"), rs.getInt("nbh.so_dien_thoai"), rs.getString("nbh.dia_chi"),rs.getDouble("no_ccc"),rs.getString("ghi_chu"));
			...
			ASanPham vatLieu=new ASanPham(maVL, tenSanPham, mota, donViTinh, donGia, soLuong, type, new ArrayList<String>(), giaNhap);
...
public List<ChiTietLichSuMuaHang> getChiTiet(String idLichSu){
	...
	try {
		connection=ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("select * from ct_lsnh ct inner join vat_lieu m on m.id=ct.id_vl where ct.id_ls=? and ct.so_luong <>0");
...
public boolean addChiTiet(String idLichSu,String idVatLieu,double soLuong) {
	try {
		...
		if(ctOld!=null) {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("update ct_lsnh ct set ct.so_luong=? where ct.id_ls=? and ct.id_vl=?");
			...
			return NhaCungCapDao.getInstance().editNCC(nbh.getId(), nbh.getTen(), nbh.getSdt(), nbh.getDiaChi(), nbh.getNoCCC()+vl.getDonGia()*soLuong-ctOld.thanhTien(), nbh.getGhiChu());

		}
		}
		else {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("insert into ct_lsnh(id_ls,id_vl,so_luong) values (?,?,?)");
			ps.setString(1, idLichSu);
		...
		if(affect>0) {
			ASanPham vl=VatLieuDao.getInstance().getVatLieu(idVatLieu);
			NhaBanHang nbh=ls.getCoSoNhap();
			 return NhaCungCapDao.getInstance().editNCC(nbh.getId(), nbh.getTen(),nbh.getSdt(), nbh.getDiaChi(), nbh.getNoCCC()+vl.getDonGia()*soLuong, nbh.getGhiChu());
		}
	...
	try {
		...
		if(idVatLieuCu.equals(idVatLieu)) {
			...
			if(ps.executeUpdate()>0) {
				...
				ASanPham vl=VatLieuDao.getInstance().getVatLieu(idVatLieu);
				NhaBanHang nbh=ls.getCoSoNhap();
				return NhaCungCapDao.getInstance().editNCC(nbh.getId(), nbh.getTen(), nbh.getSdt(), nbh.getDiaChi(), nbh.getNoCCC()-ct.thanhTien()+vl.getDonGia()*soLuong, nbh.getGhiChu());
		}
			else return false;
		}
		ChiTietLichSuMuaHang ct=ls.getChiTietLichSu(idVatLieu);
		PreparedStatement ps=connection.prepareStatement("update ct_lsnh ct set ct.id_vl=?,ct.so_luong=? where ct.id_ls=? and ct.id_vl=?");
		ps.setString(3, id);
...
public LichSuMuaHang getLichSu(String idLichSu) {
	try {
		connection=ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("select * from lich_su_nhap_hang ls inner join ct_lsnh ct on ls.id=ct.id_ls inner join "
		...
		if(rs.next()) {
			...
			NhaBanHang nbh=new NhaBanHang(rs.getString("nbh.id"),rs.getString("nbh.ten"), rs.getInt("so_dien_thoai"), rs.getString("dia_chi"), rs.getDouble("no_ccc"), rs.getString("ghi_chu"));
...
public boolean setLichSu(String idNbh,Date ngayNhap,String id,String oldNbh) {
	try {
		connection =ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("update lich_su_nhap_hang ls set ls.id_nbh=?,ls.ngay_nhap=? where ls.id=?");
		...
		if(affect>0) {
			...
			boolean a=NhaCungCapDao.getInstance().editNCC(nbh1.getId(), nbh1.getTen(), nbh1.getSdt(), nbh1.getDiaChi(), nbh1.getNoCCC()+ls.thanhTien(), nbh1.getGhiChu());
			boolean b=NhaCungCapDao.getInstance().editNCC(nbh2.getId(), nbh2.getTen(), nbh2.getSdt(), nbh2.getDiaChi(), nbh2.getNoCCC()-ls.thanhTien(), nbh2.getGhiChu());
			return a&&b;
		}
	} catch (SQLException e) {
		// TODO Auto-generated catch block
...
public boolean addLichSu(String idls,String idNbh,Date ngayNhap) {
	try {
		connection =ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("insert into lich_su_nhap_hang(id,ngay_nhap,id_nbh,hd) values(?,?,?,?)");
		ps.setString(1, idls);
		ps.setDate(2, ngayNhap);
		...
		if(affect>0) {
			...
			if(nbh!=null) {
				return addChiTiet&&NhaCungCapDao.getInstance().editNCC(nbh.getId(), nbh.getTen(), nbh.getSdt(), nbh.getDiaChi(), nbh.getNoCCC(), nbh.getGhiChu());
			}
			return false;
		}
...
public boolean deleteLichSu(String idLichSu) {
	...
	if(affect>0) {
		NhaBanHang nbh=ls.getCoSoNhap();
		return NhaCungCapDao.getInstance().editNCC(nbh.getId(), nbh.getTen(), nbh.getSdt(), nbh.getDiaChi(), nbh.getNoCCC()-ls.thanhTien(), nbh.getGhiChu());
	}
	} catch (SQLException e) {
...
public boolean deleteChiTiet(String idLs,String idVl) {
	try {
		...
		connection=ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("delete from ct_lsnh ct where ct.id_ls=? and ct.id_vl=?");
		ps.setString(1, idLs);
		ps.setString(2, idVl);
	int affect=ps.executeUpdate();
	if(affect>0) {
...
public int totalReceived (int month, int year) {
	try {
		connection =ConnectionDao.getConnection();
		PreparedStatement ps=connection.prepareStatement("SELECT COUNT(*) as total from  lich_su_nhap_hang as d WHERE d.hd=1 AND YEAR(d.NGAY_NHAP) = ? AND MONTH(d.NGAY_NHAP) = ?");
		ps.setInt(1,year);
		ps.setInt(2,month);
