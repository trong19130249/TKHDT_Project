...
public class NhaCungCapDao {
	...
	public boolean editNCC(String id, String ten, int sdt, String diaChi, double noCCC, String ghiChu) {
		try {
			...
			PreparedStatement ps = connection.prepareStatement(
					"update nha_ban_hang nbh set nbh.id=?,nbh.ten=?,nbh.so_dien_thoai=?,nbh.dia_chi=?,nbh.no_ccc=?,nbh.ghi_chu=? where nbh.id=?");
			ps.setString(1, id);
			ps.setString(2, ten);
			ps.setInt(3, sdt);
	...
	public NhaBanHang getNBH(String idNBH) {
		try {
			...
			PreparedStatement ps = connection
					.prepareStatement("select * from nha_ban_hang nbh where nbh.id=? and nbh.hd=1");
	...
	public boolean addNCC(String id, String ten, int sdt, String diaChi, double noCCC, String ghiChu) {
		try {
			connection = ConnectionDao.getConnection();
			PreparedStatement ps = connection.prepareStatement(
					"insert into nha_ban_hang(id,ten,so_dien_thoai,dia_chi,no_ccc,ghi_chu,hd) values(?,?,?,?,?,?,?)");
			ps.setString(1, id);
			ps.setString(2, ten);
			ps.setInt(3, sdt);
			ps.setString(4, diaChi);
