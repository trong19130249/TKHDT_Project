...
public static TraNoDao getInstance() {
	if(instance==null) instance=new TraNoDao();
	return instance;
}
	public List<TraNo> getAllDebt(){
		...
		try {
			...
			PreparedStatement ps=connection.prepareStatement("select * from nha_ban_hang nbh inner join ls_tra_no ls on ls.id_nbh=nbh.id",
	...
	public TraNo getDebt(String idNbh){
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("select * from nha_ban_hang nbh inner join ls_tra_no ls on ls.id_nbh=nbh.id where ls.id_nbh=?",
					ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_UPDATABLE);
	...
	public boolean setDebt(String idNbh,Date ngayTra,double soTienNo,String idOld) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("update ls_tra_no ls set ls.id_nbh=?,ls.so_tien_no=?,ls.ngay_tra=? where ls.id_nbh=?");
			ps.setString(1, idNbh);
	...
	public boolean addDebt(String id,Date ngayTra,double soTienNo) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("insert into ls_tra_no(id_nbh,ngay_tra,so_tien_no) values(?,?,?)");
			ps.setString(1, id);
	...
	public boolean deleteDebt(String idNBH,Date ngayTra) {
		try {
			connection=ConnectionDao.getConnection();
			PreparedStatement ps=connection.prepareStatement("DELETE from ls_tra_no where ls_tra_no.ID_nbh=? and ls_tra_no.ngay_tra=?");
		ps.setString(1, idNBH);
		ps.setDate(2, ngayTra);
		int affect=ps.executeUpdate();
		if(affect>0) return true;
